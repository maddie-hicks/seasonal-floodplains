...
Below if the workflow for getting survey temperature logger data ready to merge (header name cleanup, date-time formatting, copying Date_Time column into Join_DateTime, converting it to a data.table, and setting the key).
...
Survey_Temp_Prep_Join_Function <- function(Survey_Temp_File_Name){
  ...
  # strsplit() allows you to split the imported .csv file name by _
  # For example 21099530_07132021_AAB_RBK_Sauk_READONLY.csv would be split into list [[1]] with the items [1] 21099530 [2] 07132021 [3] AAB [4] RBK [5] Sauk [6] READONLY
  # Specify the list number [[1]] and then assign how you want the object to be named based off of the items in the list
  
  y <- strsplit(Survey_Temp_File_Name, "_")[[1]]
  Survey_Temp_Site_Code <- y[3]
  Survey_Temp_Date <- y[2]
  Survey_Temp_Object_Name <- paste(Survey_Temp_Site_Code, "Survey_Temp", Survey_Temp_Date, sep ="_")
  
  
  # Return produced data.table Survey_Temp_Data (which we called "Data" although you don't need to name them) suggested object name (which we called "Suggested_Name)
...
Survey_Temp_File_Name <- "..\\..\\Data\\Raw Data\\Survey Temperature Logger Data\\21099530_07132021_AAB_RBK_Sauk_READONLY.csv"

out <- Survey_Temp_Prep_Join_Function(Survey_Temp_File_Name)
